...
class M365Provider(CollaborationProvider):
    ...
    async def list_channels(self, team_id: str | None = None) -> list[Channel]:
        ...
        if team_id:
            ...
            for team in (teams.value or [])[:5]:  # Limit to first 5 teams
                try:
                    channels = await self._client.teams.by_team_id(team.id).channels.get()
    ...
    async def get_messages(
        ...
        for msg in (result.value or [])[:limit]:
            ...
            messages.append(Message(
                ...
                sender=sender,
                timestamp=str(msg.created_date_time) if msg.created_date_time else "",
    ...
    async def list_documents(
        ...
        if folder_path and folder_path != "root":
            ...
            result = await self._client.sites.by_site_id(site_id).drive.root.children.get()
    ...
    async def upload_document(
        ...
        path = f"{folder_path}/{name}" if folder_path and folder_path != "root" else name
        
        result = await self._client.sites.by_site_id(
            site_id
        ).drive.root.item_with_path(path).content.put(content)
    ...
    async def download_document(
        ...
        content = await self._client.sites.by_site_id(
            site_id
        ).drive.items.by_drive_item_id(document_id).content.get()
        return content or b""
    ...
    async def list_tasks(self, plan_id: str | None = None) -> list[Task]:
        ...
        result = await self._client.planner.plans.by_planner_plan_id(plan_id).tasks.get()
